...
module.exports = function(sqlBuilder){
	...
	sqlBuilder.registerHelper('$ine', function(query/*, outerQuery, identifier*/) {
		if (!_.isBoolean(query)){
			throw new Error('$ine must always be a Boolean.');
		}
		return ''; // there is always no return, because the $ine option will be checked and generated inside the $table basic-Helper
	});
	 ...
	 * 	$define: {
	 * 		// creates a VARCHAR(50) column NOT NULL
	 * 		last_name: { $type: 'VARCHAR', $length: 50, $notNull: true, $default: 'John' },
	 * 	}
	 *	...
	 * }
	 * ```
