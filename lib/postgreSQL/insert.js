...
module.exports = function(sqlBuilder){
	 ...
	 * @postgres true
	 *
	 * @param checkColumns 	{String | Array of Strings}		Specifies the Columns to check
 	 */
	...
	sqlBuilder.registerHelper('$checkConstraint', function(checkConstraint/*, outerQuery, identifier*/){
		...
		return 'ON CONSTRAINT ' + this.quote(checkConstraint);
	});
	...
	sqlBuilder.registerHelper('$doUpdate', function(doUpdate, outerQuery, identifier){
		...
		try {
			return 'DO UPDATE ' + sqlBuilder.callHelper('$set', doUpdate, outerQuery, identifier);
		} catch (e) {
